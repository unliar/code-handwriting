# 目录

## [数组](/leetcode/array/)

### 1. [Top k 问题](/leetcode/array/#_1-top-k-问题)

### 2. [两个栈实现队列](/leetcode/array/#_2-两个栈实现队列)

### 3. [二分查找](/leetcode/array/#_3-二分查找)

### 4. [从数组 min 到 max 中选取 n 个数](/leetcode/array/#_4-从数组-min-到-max-中选取-n-个数)

### 5. [剑指 Offer 03. 数组中重复的数字](/leetcode/array/#_5-剑指-offer-03-数组中重复的数字)

### 6. [剑指 Offer 04. 二维数组中的查找](/leetcode/array/#_6-剑指-offer-04-二维数组中的查找)

### 7. [剑指 Offer 09. 用两个栈实现队列](/leetcode/array/#_7-剑指-offer-09-用两个栈实现队列)

### 8. [剑指 Offer 11. 旋转数组的最小数字](/leetcode/array/#_8-剑指-offer-11-旋转数组的最小数字)

### 9. [剑指 Offer 21. 调整数组顺序使奇数位于偶数前面](/leetcode/array/#_9-剑指-offer-21-调整数组顺序使奇数位于偶数前面)

### 10. [剑指 Offer 29. 顺时针打印矩阵](/leetcode/array/#_10-剑指-offer-29-顺时针打印矩阵)

### 11. [剑指 Offer 39. 数组中出现次数超过一半的数字](/leetcode/array/#_11-剑指-offer-39-数组中出现次数超过一半的数字)

### 12. [剑指 Offer 40. 最小的 k 个数](/leetcode/array/#_12-剑指-offer-40-最小的-k-个数)

### 13. [剑指 Offer 42. 连续子数组的最大和](/leetcode/array/#_13-剑指-offer-42-连续子数组的最大和)

### 14. [剑指 Offer 45. 把数组排成最小的数](/leetcode/array/#_14-剑指-offer-45-把数组排成最小的数)

### 15. [剑指 Offer 49. 丑数](/leetcode/array/#_15-剑指-offer-49-丑数)

### 16. [剑指 Offer 53 - I. 在排序数组中查找数字 I](/leetcode/array/#_16-剑指-offer-53-i-在排序数组中查找数字-i)

### 17. [剑指 Offer 63. 股票的最大利润](/leetcode/array/#_17-剑指-offer-63-股票的最大利润)

### 18. [字符串翻转](/leetcode/array/#_18-字符串翻转)

### 19. [常见排序算法](/leetcode/array/#_19-常见排序算法)

### 20. [数组原地去重](/leetcode/array/#_20-数组原地去重)

### 21. [数组拍平](/leetcode/array/#_21-数组拍平)

### 22. [数组数字奇数偶数排序](/leetcode/array/#_22-数组数字奇数偶数排序)

### 23. [旋转二维数组](/leetcode/array/#_23-旋转二维数组)

### 24. [有序数组是否是子序列](/leetcode/array/#_24-有序数组是否是子序列)

### 25. [面试题 10.02. 变位词组](/leetcode/array/#_25-面试题-10-02-变位词组)

## [二叉树](/leetcode/binary-tree/)

### 1. [二叉搜索树查找](/leetcode/binary-tree/#_1-二叉搜索树查找)

### 2. [二叉树的层序遍历](/leetcode/binary-tree/#_2-二叉树的层序遍历)

### 3. [二叉树的遍历](/leetcode/binary-tree/#_3-二叉树的遍历)

### 4. [剑指 Offer 27. 二叉树的镜像](/leetcode/binary-tree/#_4-剑指-offer-27-二叉树的镜像)

### 5. [剑指 Offer 28. 对称的二叉树](/leetcode/binary-tree/#_5-剑指-offer-28-对称的二叉树)

### 6. [剑指 Offer 54. 二叉搜索树的第 k 大节点](/leetcode/binary-tree/#_6-剑指-offer-54-二叉搜索树的第-k-大节点)

## [动态规划](/leetcode/dp/)

### 1. [剑指 Offer 42. 连续子数组的最大和](/leetcode/dp/#_1-剑指-offer-42-连续子数组的最大和)

### 2. [打印n对括号的组合](/leetcode/dp/#_2-打印n对括号的组合)

### 3. [最长递增子序列](/leetcode/dp/#_3-最长递增子序列)

### 4. [爬楼梯或者斐波那契数列](/leetcode/dp/#_4-爬楼梯或者斐波那契数列)

## [前端常见八股题](/leetcode/js/)

### 1. [a==1&&a==2&&a==3](/leetcode/js/#_1-a-1-a-2-a-3)

### 2. [add(1)(2)(3)](/leetcode/js/#_2-add-1-2-3)

### 3. [async await 原理实现](/leetcode/js/#_3-async-await-原理实现)

### 4. [bind-call-apply](/leetcode/js/#_4-bind-call-apply)

### 5. [LazyMan](/leetcode/js/#_5-lazyman)

### 6. [lru](/leetcode/js/#_6-lru)

### 7. [实现 instanceOf](/leetcode/js/#_7-实现-instanceof)

### 8. [实现 koa compose](/leetcode/js/#_8-实现-koa-compose)

### 9. [实现 new 和 create](/leetcode/js/#_9-实现-new-和-create)

### 10. [实现 promise 的并发控制](/leetcode/js/#_10-实现-promise-的并发控制)

### 11. [实现 url query parse](/leetcode/js/#_11-实现-url-query-parse)

### 12. [实现一个简单的express-middleware](/leetcode/js/#_12-实现一个简单的express-middleware)

### 13. [手写 Promise.all](/leetcode/js/#_13-手写-promise-all)

### 14. [手写 Promise](/leetcode/js/#_14-手写-promise)

### 15. [数组对象深度遍历获取 id](/leetcode/js/#_15-数组对象深度遍历获取-id)

### 16. [数组转树结构](/leetcode/js/#_16-数组转树结构)

### 17. [斐波那契数列](/leetcode/js/#_17-斐波那契数列)

### 18. [树组件删除](/leetcode/js/#_18-树组件删除)

### 19. [深拷贝](/leetcode/js/#_19-深拷贝)

### 20. [防抖节流](/leetcode/js/#_20-防抖节流)

## [链表](/leetcode/linked-list/)

### 1. [删除排序链表中的重复元素](/leetcode/linked-list/#_1-删除排序链表中的重复元素)

### 2. [判断是否是环形链表](/leetcode/linked-list/#_2-判断是否是环形链表)

### 3. [剑指 Offer 06. 从尾到头打印链表](/leetcode/linked-list/#_3-剑指-offer-06-从尾到头打印链表)

### 4. [剑指 Offer 18. 删除链表的节点](/leetcode/linked-list/#_4-剑指-offer-18-删除链表的节点)

### 5. [剑指 Offer 22. 链表中倒数第 k 个节点](/leetcode/linked-list/#_5-剑指-offer-22-链表中倒数第-k-个节点)

### 6. [剑指 Offer 24. 反转链表](/leetcode/linked-list/#_6-剑指-offer-24-反转链表)

### 7. [剑指 Offer 25. 合并两个排序的链表](/leetcode/linked-list/#_7-剑指-offer-25-合并两个排序的链表)

### 8. [链表旋转 k 次](/leetcode/linked-list/#_8-链表旋转-k-次)

## [数字](/leetcode/number/)

### 1. [x 的 n 次方](/leetcode/number/#_1-x-的-n-次方)

### 2. [x 的平方根](/leetcode/number/#_2-x-的平方根)

### 3. [从 n 个数中同概率的选出 m 个数字](/leetcode/number/#_3-从-n-个数中同概率的选出-m-个数字)

### 4. [剑指 Offer 10- I. 斐波那契数列](/leetcode/number/#_4-剑指-offer-10-i-斐波那契数列)

### 5. [剑指 Offer 16. 数值的整数次方](/leetcode/number/#_5-剑指-offer-16-数值的整数次方)

### 6. [剑指 Offer 17. 打印从 1 到最大的 n 位数](/leetcode/number/#_6-剑指-offer-17-打印从-1-到最大的-n-位数)

### 7. [剑指 Offer 49. 丑数](/leetcode/number/#_7-剑指-offer-49-丑数)

### 8. [剑指 Offer 64. 求 1+2+…+n](/leetcode/number/#_8-剑指-offer-64-求-1-2-n)

### 9. [射击运动员10枪90环的所有可能](/leetcode/number/#_9-射击运动员10枪90环的所有可能)

### 10. [整数反转](/leetcode/number/#_10-整数反转)

### 11. [是否是质数](/leetcode/number/#_11-是否是质数)

### 12. [爬楼梯问题](/leetcode/number/#_12-爬楼梯问题)

## [剑指 Offer 专题](/leetcode/Offer/)

### 1. [剑指 Offer 03. 数组中重复的数字](/leetcode/Offer/#剑指-offer-03-数组中重复的数字)

### 2. [剑指 Offer 04. 二维数组中的查找](/leetcode/Offer/#剑指-offer-04-二维数组中的查找)

### 3. [剑指 Offer 05. 替换空格](/leetcode/Offer/#剑指-offer-05-替换空格)

### 4. [剑指 Offer 06. 从尾到头打印链表](/leetcode/Offer/#剑指-offer-06-从尾到头打印链表)

### 5. [剑指 Offer 09. 用两个栈实现队列](/leetcode/Offer/#剑指-offer-09-用两个栈实现队列)

### 6. [剑指 Offer 11. 旋转数组的最小数字](/leetcode/Offer/#剑指-offer-11-旋转数组的最小数字)

### 7. [剑指 Offer 15. 二进制中 1 的个数](/leetcode/Offer/#剑指-offer-15-二进制中-1-的个数)

### 8. [剑指 Offer 16. 数值的整数次方](/leetcode/Offer/#剑指-offer-16-数值的整数次方)

### 9. [剑指 Offer 17. 打印从 1 到最大的 n 位数](/leetcode/Offer/#剑指-offer-17-打印从-1-到最大的-n-位数)

### 10. [剑指 Offer 18. 删除链表的节点](/leetcode/Offer/#剑指-offer-18-删除链表的节点)

### 11. [剑指 Offer 21. 调整数组顺序使奇数位于偶数前面](/leetcode/Offer/#剑指-offer-21-调整数组顺序使奇数位于偶数前面)

### 12. [剑指 Offer 22. 链表中倒数第 k 个节点](/leetcode/Offer/#剑指-offer-22-链表中倒数第-k-个节点)

### 13. [剑指 Offer 24. 反转链表](/leetcode/Offer/#剑指-offer-24-反转链表)

### 14. [剑指 Offer 25. 合并两个排序的链表](/leetcode/Offer/#剑指-offer-25-合并两个排序的链表)

### 15. [剑指 Offer 27. 二叉树的镜像](/leetcode/Offer/#剑指-offer-27-二叉树的镜像)

### 16. [剑指 Offer 28. 对称的二叉树](/leetcode/Offer/#剑指-offer-28-对称的二叉树)

### 17. [剑指 Offer 29. 顺时针打印矩阵](/leetcode/Offer/#剑指-offer-29-顺时针打印矩阵)

### 18. [剑指 Offer 38. 字符串的排列](/leetcode/Offer/#剑指-offer-38-字符串的排列)

### 19. [剑指 Offer 39. 数组中出现次数超过一半的数字](/leetcode/Offer/#剑指-offer-39-数组中出现次数超过一半的数字)

### 20. [剑指 Offer 40. 最小的 k 个数](/leetcode/Offer/#剑指-offer-40-最小的-k-个数)

### 21. [剑指 Offer 42. 连续子数组的最大和](/leetcode/Offer/#剑指-offer-42-连续子数组的最大和)

### 22. [剑指 Offer 45. 把数组排成最小的数](/leetcode/Offer/#剑指-offer-45-把数组排成最小的数)

### 23. [剑指 Offer 48. 最长不含重复字符的子字符串](/leetcode/Offer/#剑指-offer-48-最长不含重复字符的子字符串)

### 24. [剑指 Offer 49. 丑数](/leetcode/Offer/#剑指-offer-49-丑数)

### 25. [剑指 Offer 50. 第一个只出现一次的字符](/leetcode/Offer/#剑指-offer-50-第一个只出现一次的字符)

### 26. [剑指 Offer 53 - I. 在排序数组中查找数字 I](/leetcode/Offer/#剑指-offer-53-i-在排序数组中查找数字-i)

### 27. [剑指 Offer 54. 二叉搜索树的第 k 大节点](/leetcode/Offer/#剑指-offer-54-二叉搜索树的第-k-大节点)

### 28. [剑指 Offer 63. 股票的最大利润](/leetcode/Offer/#剑指-offer-63-股票的最大利润)

### 29. [剑指 Offer 64. 求 1+2+…+n](/leetcode/Offer/#剑指-offer-64-求-1-2-n)

### 30. [剑指 Offer 10- I. 斐波那契数列](/leetcode/Offer/#剑指-offer-10-i-斐波那契数列)

## [字符串](/leetcode/string/)

### 1. [二进制数中 1 的个数](/leetcode/string/#_1-二进制数中-1-的个数)

### 2. [判断是否是回文数](/leetcode/string/#_2-判断是否是回文数)

### 3. [剑指 Offer 05. 替换空格](/leetcode/string/#_3-剑指-offer-05-替换空格)

### 4. [剑指 Offer 15. 二进制中 1 的个数](/leetcode/string/#_4-剑指-offer-15-二进制中-1-的个数)

### 5. [剑指 Offer 38. 字符串的排列](/leetcode/string/#_5-剑指-offer-38-字符串的排列)

### 6. [剑指 Offer 48. 最长不含重复字符的子字符串](/leetcode/string/#_6-剑指-offer-48-最长不含重复字符的子字符串)

### 7. [剑指 Offer 50. 第一个只出现一次的字符](/leetcode/string/#_7-剑指-offer-50-第一个只出现一次的字符)

### 8. [大数相加](/leetcode/string/#_8-大数相加)

### 9. [字符串最长不连续子串](/leetcode/string/#_9-字符串最长不连续子串)

### 10. [字符串的全排列](/leetcode/string/#_10-字符串的全排列)